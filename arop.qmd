...
arope_indi_correlations <- rr_silc_2023 |> 
  ...
  select(arope, eu_arope2, under_national_arop, under_arop, low_life_satisfaction, severe_material_social_deprivation, bad_or_very_bad_health) |> 
  correlate() |> 
  as.data.frame()
...
```{r}
hh_r_silc_2023_national_arop |> 
  group_by(country, low_work_intensity) |>
  summarise(
    mean_income = wtd.mean(income_disposable_eqi, hh_cross_weight, 
    na.rm = TRUE), 
    median_income = wtd.quantile(income_disposable_eqi, hh_cross_weight, 0.5,
    ), 
    national_arop_boundary = head(national_arop_boundary, 1)
    )
```
...
hh_r_silc_2023_arop |> 
  mutate(
    ...
    material_social_deprivation = sum_deprived_items >= 5, 
    paq_poor = under_eu_poverty | material_social_deprivation, 
    arope = at_risk_of_poverty_or_social_exclusion >= 100
  ) |> 
  select(paq_poor, arope, low_life_satisfaction, bad_or_very_bad_health) |>
  mutate(across(where(is.logical), as.numeric)) |>
  correlate()
```
