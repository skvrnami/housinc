...
arope_split2 |> 
  ...
  ungroup() |> 
  mutate(pct = pct * 100) |> 
  mutate(country = factor(country, levels = rev(c(as.character(arope_split_countries2), "EU")))) |>
  arrange(country) |> 
  knitr::kable(digits = 1, col.names = c("Country", "% of households in EU AROPE"))
```
...
full_join(
  aropes3, 
  aropes3b |> 
  pivot_wider(names_from = indicator, values_from = pct), by = "country") |> 
  rename(`Země` = country, 
         `Pod 50 % evropského mediánu nebo v materiální/sociální deprivaci` = pct_eu_arope) |>
  kable(digits = 2)
```
