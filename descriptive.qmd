...
hh_r_silc_2023 <- hh_r_silc_2023 |> 
    ...
    mutate(
        income_disposable_wo_hb_eqi = (income_disposable - allowance_housing) / consumption_units, 
        income_disposable_wo_hb = (income_disposable - allowance_housing)
    )
...
hh_r_silc_2023 <- hh_r_silc_2023 |> 
    ...
    mutate(
      income_disposable_quantile_wo_hb = case_when(
        income_disposable_wo_hb_eqi <= p20 ~ "1st quantile (lowest)",
        income_disposable_wo_hb_eqi <= p40 ~ "2nd quantile",
        income_disposable_wo_hb_eqi <= p60 ~ "3rd quantile",
        income_disposable_wo_hb_eqi <= p80 ~ "4th quantile",
        income_disposable_wo_hb_eqi > p80 ~ "5th quantile (highest)"
...
hb_total <- hh_r_silc_2023 |> 
    group_by(country) |> 
    summarise(
        `% of HH taking housing allowance (total)` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100)
...
hb_tenants <- hh_r_silc_2023 |> 
    ...
    group_by(country) |> 
    summarise(
        `% of HH taking housing allowance (tenants)` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100)
...
hb_tenants_market <- hh_r_silc_2023 |> 
    ...
    group_by(country) |> 
    summarise(
        `% of HH taking housing allowance (tenants at market price)` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100)
...
hb_tenants_reduced <- hh_r_silc_2023 |> 
    ...
    group_by(country) |> 
    summarise(
        `% of HH taking housing allowance (tenants at reduced price)` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100)
...
hh_r_silc_2023 |> 
    ...
    group_by(country, income_disposable_quantile_wo_hb) |>
    summarise(
        `% of HH taking housing allowance` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100
    ) |> 
    pivot_wider(names_from = income_disposable_quantile_wo_hb,
                values_from = `% of HH taking housing allowance`) |> 
...
hh_r_silc_2023 |> 
    ...
    mutate(
        ...
        rent_to_income_after_ha = (rent - (allowance_housing / 12)) / (income_disposable_wo_hb / 12)
    ) |>
    group_by(country) |>
    summarise(
...
hh_r_silc_2023 |> 
    ...
    mutate(
        ...
        housing_to_income_after_ha = (total_housing_cost - (allowance_housing / 12)) / (income_disposable_wo_hb / 12)
    ) |>
    group_by(country) |>
    summarise(
...
select_countries <- function(df){
    df |> 
        ...
        filter(country %in% c("CZ", "FI", "RO", "IT", "NL", "BE", "DE", "EE")) |> 
        mutate(
        allowance_housing = case_when(
            country == "DE" & is.na(allowance_housing) ~ 0,
            country == "FI" & is.na(allowance_housing) ~ 0,
            country == "NL" & is.na(allowance_housing) ~ 0,
            TRUE ~ allowance_housing
        )
        ) |>
        mutate(
            income_disposable_wo_hb_eqi = (income_disposable - allowance_housing) / consumption_units, 
...
population_stats <- hh_all_silc |> 
    ...
    summarise(
      `% receiving benefits` = wtd.mean(allowance_housing > 0, weights = hh_cross_weight) * 100,
      `mean benefit amount (population)` = wtd.mean(allowance_housing, weights = hh_cross_weight),
...
ggplot(population_stats,
    ...
    geom_point() +
    labs(
      title = "Difference in median housing cost-to-income ratio before and after housing allowance over time",
      x = "Year",
...
ggplot(population_stats_q,
    ...
    geom_point() +
    labs(
      title = "% of households receiving housing allowances over time by income quantile",
      x = "Year",
      y = "% of households receiving housing allowances",
