...
calculate_dimensions <- function(df){
  df %>% 
    mutate(
      ...
      dim_insecurity = arrears_mortgage_rent == 1 |
          arrears_utility == 1,
...
summary(m_out)
cz_m_summary <- summary(m_out)
cz_m_summary$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/cz_matching_data.html")
plot(summary(m_out))
...
table(m_data$weights)

eligibility_cz_2023 |>
  filter(!is.na(region)) |> 
  summarise(
    across(
      c(dim_affordability, dim_insecurity, dim_quality,
        arrears_mortgage_rent, arrears_utility,
        overcrowded_eurostat, inability_to_keep_warm), 
      ~wtd.mean(.x, weights = hh_cross_weight, na.rm = TRUE) * 100)) |> 
  knitr::kable(
    digits = 1, caption = "Whole population", 
    col.names = c(
      "Unaffordability", "Insecurity", "Poor quality", 
      "Arrears on mortgage/rent", "Arrears on utilities", 
      "Overcrowded", "Unable to keep warm"
    ))
...
eligibility_cz_2023 |>
  ...
  summarise(
    across(
      ...
      ~wtd.mean(.x, weights = hh_cross_weight, na.rm = TRUE) * 100)) |> 
  knitr::kable(
    digits = 1, caption = "Whole population", format = "html", 
      col.names = c(
      "Unaffordability", "Insecurity", "Poor quality", 
      "Arrears on mortgage/rent", "Arrears on utilities", 
      "Overcrowded", "Unable to keep warm"
    )) |> 
  writeLines("paper/figs/cz_precarity_population.html")
  ...
  summarise(
    across(
      c(dim_affordability, dim_insecurity, dim_quality,
        ...
        overcrowded_eurostat, inability_to_keep_warm), 
      ~wtd.mean(.x, weights = hh_cross_weight, na.rm = TRUE) * 100)) |> 
  knitr::kable(digits = 1, caption = "Matched households", format = "html", col.names = c(
      "Group",
      "Unaffordability", "Insecurity", "Poor quality", 
      "Arrears on mortgage/rent", "Arrears on utilities", 
      "Overcrowded", "Unable to keep warm"
    )) |> 
  writeLines("paper/figs/cz_precarity_matched.html")
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2, 
  output = "paper/figs/cz_matching_summary.html"
)
```
...
m_data |> 
  mutate(
    rent = if_else(is.na(rent), 0, rent), 
    total_disposable_income_before_social_transfers = 
      if_else(total_disposable_income_before_social_transfers < 0, 0, total_disposable_income_before_social_transfers)
  ) |> 
  group_by(takeup_allowance_housing) |> 
  mutate(
    share_income_on_housing = total_housing_cost / total_disposable_income_before_social_transfers
  ) |> 
  mutate(
    share_income_on_housing = if_else(share_income_on_housing > 1, 1, share_income_on_housing)
  ) |> 
  summarise(
    # mean_rent = mean(rent, na.rm = TRUE), 
    mean_rent = mean(rent),
    mean_thc = mean(total_housing_cost, na.rm = TRUE), 
    mean_thc_share = mean(share_income_on_housing)
  )
```
...
m_out_robust <- matchit(takeup_allowance_housing ~ n_persons + 
                   income_disposable_wo_housing + 
                   eligibility_allowance_housing + 
                   region + tenure_status + 
                   mean_work_intensity + work_intensity_not_defined, 
...
cz_m_summary_robust <- summary(m_out_robust)
cz_m_summary_robust$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/cz_matching_data_robust.html")
plot(summary(m_out_robust), var.order = "unmatched")

m_data_robust <- match_data(m_out_robust) %>% 
  mutate(takeup_allowance_housing = factor(takeup_allowance_housing, 
                                               levels = c(FALSE, TRUE), 
                                               labels = c("No, not taking benefits",
                                                          "Yes, taking benefits")), 
...
## Matched data
cz_affordability2 <- glm(dim_affordability ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_insecurity2 <- glm(dim_insecurity ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_arrears_mortgage_rent2 <- glm(arrears_mortgage_rent ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_arrears_utility2 <- glm(arrears_utility ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_quality2 <- glm(dim_quality ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_overcrowded2 <- glm(overcrowded_eurostat ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)

cz_warm2 <- glm(inability_to_keep_warm ~ takeup_allowance_housing + eligibility_allowance_housing + 
      income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = m_data_robust)
...
# pr2 <- data.frame(
#   label = "pseudo-R2",
#   x1 = pscl::pR2(cz_affordability2)["McFadden"], 
#   x2 = pscl::pR2(cz_insecurity2)["McFadden"],
#   x3 = pscl::pR2(cz_quality2)["McFadden"],
#   x4 = pscl::pR2(cz_arrears_mortgage_rent2)["McFadden"],
#   x5 = pscl::pR2(cz_arrears_utility2)["McFadden"],
#   x6 = pscl::pR2(cz_overcrowded2)["McFadden"],
#   x7 = pscl::pR2(cz_warm2)["McFadden"]
# )

modelsummary::modelsummary(
  list(
    "Unaffordability" = cz_affordability2, 
    "Insecurity" = cz_insecurity2, 
    "Poor quality" = cz_quality2, 
    "Arrears on mortgage/rent" = cz_arrears_mortgage_rent2, 
    "Arrears on utilities" = cz_arrears_utility2, 
    "Overcrowded" = cz_overcrowded2, 
    "Unable to keep warm" = cz_warm2
  ), 
  stars = TRUE, 
  fmt = 2,
  coef_rename = c(
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2,
  output = "paper/figs/cz_matching_summary_robust.html"
)
```
...
summary(nl_m_out)
nl_m_summary <- summary(nl_m_out)
nl_m_summary$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/nl_matching_data.html")
...
eligibility_nl_2023 |>
  ...
  writeLines("paper/figs/nl_precarity_population.html")
...
nl_m_data |> 
  ...
  writeLines("paper/figs/nl_precarity_matched.html")
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2,
  output = "paper/figs/nl_matching_summary.html"
)
```
...
nl_m_out_robust <- matchit(takeup_allowance_housing ~ n_persons + 
                   income_disposable_wo_housing + 
                   eligibility_allowance_housing + 
                   tenure_status + 
                   mean_work_intensity + work_intensity_not_defined, 
...
summary(nl_m_out_robust)
nl_m_summary_robust <- summary(nl_m_out_robust)
nl_m_summary_robust$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/nl_matching_data_robust.html")
plot(summary(nl_m_out_robust))

nl_m_data_robust <- match_data(nl_m_out_robust) %>% 
  mutate(takeup_allowance_housing = factor(takeup_allowance_housing, 
                                               levels = c(FALSE, TRUE), 
                                               labels = c("No, not taking benefits",
                                                          "Yes, taking benefits")), 
...
nl_affordability2 <- glm(dim_affordability ~ takeup_allowance_housing, # + 
  # eligibility_allowance_housing + 
  #     income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
    data = nl_m_data_robust)

nl_insecurity2 <- glm(dim_insecurity ~ takeup_allowance_housing, 
    # + eligibility_allowance_housing + 
    #   income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
nl_arrears_mortgage_rent2 <- glm(arrears_mortgage_rent ~ takeup_allowance_housing,
#  + eligibility_allowance_housing + 
      # income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
nl_arrears_utility2 <- glm(arrears_utility ~ takeup_allowance_housing,
#  + eligibility_allowance_housing + 
#       income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
nl_quality2 <- glm(dim_quality ~ takeup_allowance_housing, 
# + eligibility_allowance_housing + 
#       income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
nl_overcrowded2 <- glm(overcrowded_eurostat ~ takeup_allowance_housing,
#  + eligibility_allowance_housing + 
#       income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
nl_warm2 <- glm(inability_to_keep_warm ~ takeup_allowance_housing,
#  + eligibility_allowance_housing + 
#       income_disposable_wo_housing, 
    family = "binomial",
    weights = weights, 
...
# pr2 <- data.frame(
#   label = "pseudo-R2",
#   x1 = pscl::pR2(nl_affordability2)["McFadden"], 
#   x2 = pscl::pR2(nl_insecurity2)["McFadden"],
#   x3 = pscl::pR2(nl_quality2)["McFadden"],
#   x4 = pscl::pR2(nl_arrears_mortgage_rent2)["McFadden"],
#   x5 = pscl::pR2(nl_arrears_utility2)["McFadden"],
#   x6 = pscl::pR2(nl_overcrowded2)["McFadden"],
#   x7 = pscl::pR2(nl_warm2)["McFadden"]
# )

modelsummary::modelsummary(
  list(
    "Unaffordability" = nl_affordability2, 
    "Insecurity" = nl_insecurity2, 
    "Poor quality" = nl_quality2, 
    "Arrears on mortgage/rent" = nl_arrears_mortgage_rent2, 
    "Arrears on utilities" = nl_arrears_utility2, 
    "Overcrowded" = nl_overcrowded2, 
    "Unable to keep warm" = nl_warm2
  ), 
  stars = TRUE, 
  fmt = 2,
  coef_rename = c(
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2,
  output = "paper/figs/nl_matching_summary_robust.html"
)
...
fi_match_summary <- summary(fi_allowance_2023_match)
fi_match_summary$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/fi_matching_data.html")
  ...
  summarise(
    across(
      c(dim_affordability, dim_insecurity, dim_quality,
        ...
        overcrowded_eurostat, inability_to_keep_warm), 
      ~wtd.mean(.x, weights = hh_cross_weight, na.rm = TRUE) * 100)) |> 
  knitr::kable(digits = 1, caption = "Whole population", col.names = c(
      "Unaffordability", "Insecurity", "Poor quality", 
      "Arrears on mortgage/rent", "Arrears on utilities", 
      "Overcrowded", "Unable to keep warm"
    ))
...
eligibility_fi_2023 |>
  ...
  writeLines("paper/figs/fi_precarity_population.html")
  ...
  summarise(
    across(
      c(dim_affordability, dim_insecurity, dim_quality,
        ...
        overcrowded_eurostat, inability_to_keep_warm), 
      ~wtd.mean(.x, weights = hh_cross_weight, na.rm = TRUE) * 100)) |> 
  knitr::kable(digits = 1, caption = "Matched households", col.names = c(
      "Group", 
      "Unaffordability", "Insecurity", "Poor quality", 
      "Arrears on mortgage/rent", "Arrears on utilities", 
      "Overcrowded", "Unable to keep warm"
...
fi_general_m_data |> 
  ...
  writeLines("paper/figs/fi_precarity_matched.html")
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2,
  output = "paper/figs/fi_matching_summary.html"
)
...
summary(fi_allowance_2023_match)
fi_match_robust <- summary(fi_allowance_2023_match)
fi_match_robust$sum.matched |> 
  as.data.frame() |> 
  select(`Means Treated`, `Means Control`, `Std. Mean Diff.`) |>
  knitr::kable(digits = 2, format = "html") |> 
  writeLines("paper/figs/fi_matching_data_robust.html")
plot(summary(fi_allowance_2023_match))

fi_general_m_data_robust <- match_data(fi_allowance_2023_match) %>% 
  mutate(takeup_allowance_housing = factor(takeup_allowance_housing, 
                                               levels = c(FALSE, TRUE), 
                                               labels = c("No, not taking benefits",
                                                          "Yes, taking benefits")), 
...
fi_affordability2 <- glm(dim_affordability ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  data = fi_general_m_data_robust)

fi_insecurity2 <- glm(dim_insecurity ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
fi_arrears_mortgage_rent2 <- glm(arrears_mortgage_rent ~ takeup_allowance_housing +
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
fi_arrears_utility2 <- glm(arrears_utility ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
fi_quality2 <- glm(dim_quality ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
fi_overcrowded2 <- glm(overcrowded_eurostat ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
fi_warm2 <- glm(inability_to_keep_warm ~ takeup_allowance_housing + 
  eligibility_allowance_housing + 
  income_disposable_wo_housing, 
  family = "binomial",
  weights = weights, 
...
# pr2 <- data.frame(
#   label = "pseudo-R2",
#   x1 = pscl::pR2(fi_affordability2)["McFadden"], 
#   x2 = pscl::pR2(fi_insecurity2)["McFadden"],
#   x3 = pscl::pR2(fi_quality2)["McFadden"],
#   x4 = pscl::pR2(fi_arrears_mortgage_rent2)["McFadden"],
#   x5 = pscl::pR2(fi_arrears_utility2)["McFadden"],
#   x6 = pscl::pR2(fi_overcrowded2)["McFadden"],
#   x7 = pscl::pR2(fi_warm2)["McFadden"]
# )

modelsummary::modelsummary(
  list(
    "Unaffordability" = fi_affordability2, 
    "Insecurity" = fi_insecurity2, 
    "Poor quality" = fi_quality2, 
    "Arrears on mortgage/rent" = fi_arrears_mortgage_rent2, 
    "Arrears on utilities" = fi_arrears_utility2, 
    "Overcrowded" = fi_overcrowded2, 
    "Unable to keep warm" = fi_warm2
  ), 
  stars = TRUE, 
  fmt = 2,
  coef_rename = c(
...
modelsummary::modelsummary(
  ...
  gof_map = "nobs", 
  # add_rows = pr2, 
  output = "paper/figs/fi_matching_summary_robust.html"
)
...
eligibility_de_2023 <- de_2023 |> 
  mutate(
    max_rent = case_when(
      ...
      n_persons == 4 ~ 1057.2,
      n_persons == 5 ~ 1208.6,
      n_persons == 6 ~ 1208.6 + 146.4
    ),
    total_housing_cost_capped = if_else(total_housing_cost > max_rent, max_rent, total_housing_cost),
    month_income = (income_disposable - allowance_housing) / 12,
    a = case_when(
      n_persons == 1 ~ 4e-2, 
      n_persons == 2 ~ 3e-2,
    ...
    b = case_when(
      n_persons == 1 ~ 4.911e-4, 
      n_persons == 2 ~ 3.716e-4,
      n_persons == 3 ~ 3.035e-4,
      n_persons == 4 ~ 2.251e-4,
      n_persons == 5 ~ 1.985e-4,
      n_persons == 6 ~ 1.792e-4
    ),
    c = case_when(
      n_persons == 1 ~ 4.620e-5, 
      n_persons == 2 ~ 3.450e-5,
      n_persons == 3 ~ 2.780e-5,
      n_persons == 4 ~ 2.000e-5,
      n_persons == 5 ~ 1.950e-5,
      n_persons == 6 ~ 1.880e-5
    ),
    eligibility_allowance_housing = 
      1.15 * (total_housing_cost_capped - (a + b * total_housing_cost_capped + c * month_income) * month_income),

  )

eligibility_de_2023 |> 
  filter(eligibility_allowance_housing < 0) |>
  select(allowance_housing, eligibility_allowance_housing) |> 
  corrr::correlate()
...
cz_plot <- modelplot(
  ...
  coef_map = c("takeup_allowance_housingYes, taking benefits"="")
...
cz_plot + nl_plot + fi_plot + plot_layout(guides = 'collect', axis_titles = "collect") + 
  plot_annotation(
    title = "Effects of taking benefits on dimensions of precarity"  
  )

ggsave("paper/figs/fig_effect_benefit_takeup.png", width = 10, height = 6, bg = "white")
```
...
cz_plot + nl_plot + fi_plot + 
  plot_layout(guides = 'collect', axis_titles = "collect") + 
  plot_annotation(
    title = "Effects of taking benefits on dimensions of precarity"  
  )

ggsave("paper/figs/fig_effect_benefit_takeup_all.png", 
...
cz_plot2 + nl_plot2 + fi_plot2 + plot_layout(guides = 'collect', axis_titles = "collect") + 
  # theme(legend.position = "top") + 
  plot_annotation(title = "Effects of taking benefits on dimensions of precarity")
  # ggtitle("Effects of taking benefits on dimensions of precarity")
