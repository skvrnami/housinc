...
all_silc_households_precarity %>% 
  filter(country %in% c("CZ", "FI", "RO", "IT", "NL", "DE", "EE")) %>%
...
ggsave("figs/fig_affordability_time.png", width = 8, height = 6, 
       bg = "white")
...
silc_merged_households %>% 
  ungroup() %>% 
  select(arrears_mortgage_rent, arrears_utility) %>% 
  table(., useNA = "always")
...
silc_merged_households %>% 
  group_by(country, year) %>% 
  summarise(share_unable_keep_warm = wtd.mean(ability_to_keep_warm == "No",
                                              hh_cross_weight, 
                                      na.rm = TRUE), 
            .groups = "drop") %>% 
  ggplot(., aes(x = year, y = share_unable_keep_warm)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~country, scales = "free_y") + 
  scale_y_continuous(labels = scales::label_percent()) + 
...
silc_merged_households %>% 
  group_by(country, year) %>% 
  summarise(share_overcrowded = wtd.mean(overcrowded_eurostat,
                                              hh_cross_weight, 
                                      na.rm = TRUE), 
            .groups = "drop") %>% 
  ggplot(., aes(x = year, y = share_overcrowded)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~country, scales = "free_y") + 
  scale_y_continuous(labels = scales::label_percent()) + 
...
silc_merged_households %>% 
  group_by(country, year) %>% 
  summarise(share_leaks_damp = wtd.mean(leaks_damp == "Yes",
                                              hh_cross_weight, 
                                      na.rm = TRUE), 
            .groups = "drop") %>% 
  ggplot(., aes(x = year, y = share_leaks_damp)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(~country, scales = "free_y") + 
  scale_y_continuous(labels = scales::label_percent()) + 
